...
{	
	_float4x4	RotationMatrix = *D3DXMatrixRotationAxis(&RotationMatrix, &vAxis, m_fRotationPerSec * fTimeDelta);
...
_bool CTransform::Chase(const _float3& vTargetPos, _float fTimeDelta, _float fLimitDistance)
{
	_float3		vMoveDir = vTargetPos - Get_State(STATE::POSITION);

	_float		fDistance = D3DXVec3Length(&vMoveDir);

	if (fDistance >= fLimitDistance)
	{
		_float3		vGoal = Get_State(STATE::POSITION) + *D3DXVec3Normalize(&vMoveDir, &vMoveDir) * m_fSpeedPerSec * fTimeDelta;
		Set_State(STATE::POSITION, vGoal);
		return true;
	}
	else
		return false;
...
void CTransform::LookAt(const _float3& vTargetPos)
{
	...
	_float3		vLook = vTargetPos - Get_State(STATE::POSITION);
